---
title: "Torque values imperial fasteners"
sidebar:
  label: "Torque values imperial fasteners"
description: "Technical reference: Torque values imperial fasteners"
...
The nominal diameters and the number of threads per inch define each imperial thread size.
...
Although ISO metric and UTS thread profiles share the 60° angle, they are not directly interchangeable. The following table provides guidance on approximate substitution between imperial and metric sizes; thread pitches and design tolerances must always be verified.
...
Mechanical properties are governed by SAE J429 for bolts from ¼ to 1 inch. The design preload is usually taken as 75% of the proof load or 70% of the ultimate tensile strength.
...
The torques shown correspond to a friction coefficient K = 0.20 (dry surfaces, black finish). For lubricated conditions, refer to the correction factors section.
...
Lubricant reduces friction between threads and bearing surface, so the tightening torque must be reduced to achieve the same preload. For lubricated bolts, it is recommended to multiply the dry torque by the following factors:
...
The recommended drill diameters for taps ensure 75% thread height, suitable for most general applications.
...
The fundamental relationship between tightening torque and clamping load is given by:
